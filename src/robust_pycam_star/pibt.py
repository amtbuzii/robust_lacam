...
class PIBT:
    ...
    def funcPIBT(
        ...
        # Prefer k-robust-safe neighbors when history is provided; fall back to all neighbors if none safe.
        C = get_neighbors_safe_k_robust(self.grid, Q_from[i], self.k_robust, i, history)
        ...
        for v in C:
            ...
            if (
                ...
                and (not self.funcPIBT(Q_from, Q_to, j, history))
            ):
                continue
    ...
    def step(
        ...
        for i, (v_i_from, v_i_to) in enumerate(zip(Q_from, Q_to)):
            ...
            if v_i_to != self.NIL_COORD:
                ...
                if self.occupied_nxt[v_i_to] != self.NIL:
                    flg_success = False
                    break
                # check edge collision TODO: check 
        ...
        # perform PIBT (history used for get_neighbors_safe_k_robust when k_robust > 0)
        if flg_success:
            for i in order:
                if Q_to[i] == self.NIL_COORD:
                    flg_success = self.funcPIBT(Q_from, Q_to, i, history)
                    if not flg_success:
                        break
