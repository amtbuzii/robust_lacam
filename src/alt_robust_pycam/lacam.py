    ...
    def _solve(self) -> Configs:
        ...
        while len(OPEN) > 0 and not self.deadline.is_expired:
            ...
            if C.depth < self.num_agents:
                ...
                if _invalid:
                    # Register intermediate configs (from k-robust conflict) so we re-open nodes when we see them
                    for _coord, intermediate in _invalid.items():
                        for Q_alt in intermediate:
                            if Q_alt in NEED_ALT:
                                if Q_alt in SEEN_ALT:
                                    N_alt = SEEN_ALT[Q_alt].pop()
                                    if not SEEN_ALT[Q_alt]:
                                        del SEEN_ALT[Q_alt]
                                    OPEN.appendleft(N_alt)
                                    NEED_ALT.remove(Q_alt)        
                            else:
                                NEED_ALT.add(Q_alt)
            ...
            if state_key in EXPLORED:
                ...
                if not self.check_k_robust(N, Q_to):
                    self.info(1, f"Node rejected: k-robustness violation for Q_to")
                    continue             
                N_new = HighLevelNode(
                    Q=Q_to,
                    parent=N,
                ...
                if not Q_only_seen or N_new.g < SEEN_CONFIGS[Q_to]:
                    OPEN.appendleft(N_new)
                    EXPLORED[state_key] = N_new
                    if Q_to not in SEEN_CONFIGS:
                        SEEN_CONFIGS[Q_to] = N_new.g
                    else:
                        SEEN_CONFIGS[Q_to] = min(SEEN_CONFIGS[Q_to], N_new.g)
                else:
                    if Q_to in NEED_ALT:
                        OPEN.appendleft(N_new)
                        EXPLORED[state_key] = N_new
                        ...
                        # avoid duplicate nodes: same state (Q, history) already in list
                        nodes_for_Q = SEEN_ALT.setdefault(Q_to, [])
                        if not any((n.Q, n.history) == state_key for n in nodes_for_Q):
                            SEEN_ALT[Q_to].append(N_new)
        ...
        if len(OPEN) == 0:
            ...
            self.info(1, f"size of seen_alt: {len(SEEN_ALT)}")
            self.info(1, f"size of explored: {len(EXPLORED)}")
            self.info(1, f"size of need_alt: {len(NEED_ALT)}")
