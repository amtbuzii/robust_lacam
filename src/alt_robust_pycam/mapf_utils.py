...
def check_coord_k_robust(
    ...
    """
    T = len(history) - 1  # index of Q_t

    if k_robust is not None and k_robust > 0:
        configs_to_check = history[max(0, len(history) - k_robust) :]
    else:
        configs_to_check = history

    for i, hist_Q in enumerate(configs_to_check):
        if cand in hist_Q and cand != hist_Q[agent_id]:
            #intermediate = list(history[i + 1 : T])
            intermediate = [history[T - 1]]
...
def get_neighbors_safe_k_robust(
    grid: Grid,
    coord: Coord,
    k: int,
    agent_id: int,
    history: tuple[Config, ...] | list[Config],
) -> tuple[list[Coord], dict[Coord, list[Config]]]:
    """
    Return (valid_neighbors, invalid_neighbors): neighbor coords (including
    current for wait) that are k-robust safe, and those that are not. When
    k==0, all candidates are valid and invalid is empty.
    """
    cands = [coord] + get_neighbors(grid, coord)
    invalid: dict[Coord, list[Config]] = {}
    if k == 0:
        return cands, invalid
    valid: list[Coord] = []
    for cand in cands:
        is_safe, intermediate = check_coord_k_robust(
            cand, agent_id, history, k_robust=k
        )
        if is_safe:
            valid.append(cand)
        else:
            invalid[cand] = intermediate
...
def export_search_tree_dot(
    ...
    # Key by state (Q, history) so one node per state; avoids multiple trees from object identity
    state_to_id: dict = {}
